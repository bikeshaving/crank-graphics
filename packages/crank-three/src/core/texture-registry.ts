/**
 * TextureRegistry - Singleton registry for managing Three.js textures with SVG-style url(#id) references
 *
 * Provides:
...
export interface PendingTextureReference {
	textureId: string;
	node: any; // Three.js object that needs the texture
	property: string; // property name to set (e.g., 'map', 'normalMap')
	resolver: (node: any, texture: THREE.Texture) => void; // function to apply the texture
}
...
export class TextureRegistry {
	 ...
	 */
	acquire(id: string): THREE.Texture | null {
	...
	unregister(id: string): void {
		const entry = this.textures.get(id);
		if (entry) {
			// Don't destroy empty textures or shared textures
			if (entry.refCount === 0) {
				try {
	...
	getDebugInfo(): Record<string, any> {
		...
		for (const [id, entry] of this.textures) {
			info[id] = {
				...
				source: entry.source,
				width: entry.texture.image?.width || 0,
				height: entry.texture.image?.height || 0,
	 ...
	 */
	private loadTexture(source: string): Promise<THREE.Texture> {
...
if (typeof window !== "undefined" && process.env.NODE_ENV === "development") {
	(window as any).__THREE_TEXTURE_REGISTRY__ = textureRegistry;
}